...
- 在全新项目目录中，让 Codex 下载仓库并按照 `docs/INSTALLATION.md` 安装 Skill。
- 当 `~/.codex/skills/oc-codex-pet-maker` 已存在且与下载版本不同时，Codex 正确停止了覆盖，但只报告路径和冲突，没有告诉用户下一步如何选择。
- 用户继续询问“下一步”后，Codex直接要求角色参考图，没有说明也可以从纯文字或现有 sprite 开始。
- 这证明现有流程只闭合了文件安装，没有闭合零背景用户的逐步引导。

### 今日完成

- 在中英文安装说明中增加三种明确分支：目标不存在、目标相同、目标不同。
- 要求安装回复必须以具体下一步或选择题结束，不能只返回路径、状态或警告。
- 目标版本不同时，必须让用户选择保留现有版本、备份后更新或先查看差异；冲突解决前不能声称新版已经可用。
- 安装成功后，明确要求新开 Codex 对话，并提供最短启动句：
  - `请使用 OC Codex Pet Maker Skill 帮我制作一个 Codex 宠物，并一步一步引导我。`
- 在 Skill 中增加强制新手引导：
  - 主动说明纯文字、参考图和现有 sprite 三种入口；
  - 不默认用户拥有参考图；
  - 开始时只要求素材起点和一句话宠物设定；
  - 标明名称、风格、配色、特征和动作偏好为可选项；
  - 每一阶段都说明已完成内容、下一步和需要用户确认的事项；
  - 保留完整提示词作为可选快捷方式，而不是使用前提。
- 同步更新中英文 README 和网站流程文案。

### 验证结果

- Skill Creator `quick_validate.py` 通过。
- 根目录与自包含 Skill 的安装说明完全一致。
- 两个 JSON 模板解析通过。
- 中英文 HTML 解析通过。
- `git diff --check` 通过。
...
1. 经用户明确同意后，备份并更新本机已安装的 Skill 副本。
2. 在新的 Codex 对话中重新测试安装冲突分支和纯文字首次制作入口。
3. 根据实测继续收紧后续各阶段的引导。
