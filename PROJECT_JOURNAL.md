...
- 已创建提交 `3e770b9`：`feat(skill): add guided beginner onboarding`。
- 已推送到 GitHub `origin/main`。
- 从 GitHub 克隆到全新测试目录，确认远端、本地新克隆和提交 `3e770b9` 一致。
- 对安装目标做了两种真实文件状态检查：
  - 旧安装副本与新克隆存在差异，对应“目标存在且不同”分支；
  - 更新后的正式安装副本与新克隆完全一致，对应“目标存在且相同”分支。
- 按用户授权保留旧安装副本备份，并将 GitHub 最新版切换到正式 Skill 安装路径。
- 确认正式安装副本包含三种素材入口、必填/选填说明、阶段交接规则和禁止只返回状态的约束。
- 尝试使用嵌套 Codex CLI 运行全新模型会话；当前桌面环境只记录了测试提示，没有返回助手回答，因此没有把该项标记为模型级通过。

### 仍需人工实测

- 在 Codex Desktop 中对新测试目录创建一个真正的新任务，只发送最短启动句。
- 预期 Skill 主动提供纯文字、参考图、现有 sprite 三种入口，并说明开始只必填素材起点和一句话宠物设定。
