...
安装完成后，新开一个 Codex 对话，只需要这样说：

```text
请使用 OC Codex Pet Maker Skill 帮我制作一个 Codex 宠物，并一步一步引导我。
```

你不需要先准备参考图，也不需要自己写完整 Prompt。Skill 应该主动说明可以从纯文字、参考图或现有 sprite 开始，然后一次只问当前步骤需要的信息。

开始制作时真正必填的只有：

- 素材起点：纯文字、参考图或现有 sprite；
- 宠物设定：你想做一个什么样的宠物，一句话也可以。

以下内容都可以稍后补充或让 Skill 提议默认值：宠物名称、视觉风格、配色、必须保留的特征和动作偏好。选择参考图或现有 sprite 时，对应文件才是必需的。

如果你希望一次提供更多信息，也可以使用下面的快捷模板：
...
1. 选择从纯文字、参考图或现有 sprite 开始。
2. 用一句话说明宠物设定；其余信息可以跳过或稍后填写。
3. Skill 说明 9 个状态，并帮助你填写或调整动作方案。
4. 等你确认后，再进入 canonical reference 和图片生成。
5. 每完成一阶段，Skill 都会说明结果、下一步和需要你确认的内容。
6. 生成横条、切图、检查预览、局部修复、合成 spritesheet，并验证宠物包。
...
这个项目来源于一次真实使用 Codex Hatch Pet Skill 的制作实践。Hatch Pet 提供从创意或参考图到宠物打包的一体化、代理编排流程；OC Codex Pet Maker 则是配套工具箱，适合希望查看中间过程、自定义方案、理解制作原理或局部修复的用户。名称保留了项目从 OC 制作出发的背景，但流程同样支持动物、吉祥物、物品和其他宠物概念。
